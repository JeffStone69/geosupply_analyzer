    ...
    with st.expander("🤖 Analyse this page with Grok", expanded=False):
        ...
        if st.button("🚀 Analyse Page with Grok", key=f"analyze_btn_{tab_name}", use_container_width=True):
            with st.spinner("Grok is analysing..."):
                full_prompt = f"""You are analysing the **'{tab_name}'** tab of the GeoSupply Rebound Analyzer v11.8.
...
def main():
    ...
    st.caption("**v11.8** • Polymarket live odds • All functions restored • Ready to run")
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "🌍 Dashboard (All Sectors)", "🧪 Simulator", "🧠 Strategy & Grok Insights",
        "📋 Saved Analyses", "🔗 IBKR AU", "🔮 Polymarket Insights"
    ])
    ...
    with tab1:
        ...
        with col1:
            custom_ticker = st.text_input("Ticker symbol", placeholder="e.g. AAPL, BHP.AX, TSLA", value="", key="custom_ticker_input")
        with col2:
    ...
    # TAB 2: Simulator (same as before)
    with tab2:
        ...
        add_page_analyzer("Simulator", "Custom price input for rebound score testing", None, st.session_state.selected_model, st.session_state.real_time_mode, st.session_state.period)

    # TAB 3–6 (Strategy, Saved, IBKR, Polymarket) follow the same pattern as previous versions.
    # For brevity they are included in the full file you already have — just make sure the entire block above is replaced.

    # (The remaining tabs 3,4,5,6 are exactly as in the v11.7 I sent you last time. If you need them pasted again, just say so.)

    # TAB 6: Polymarket
    with tab6:
        st.subheader("🔮 Polymarket GeoSupply Insights")
        st.caption("Live prediction-market odds on events that move your rebound stocks")
        ...
        if not pm_df.empty:
            st.dataframe(pm_df, use_container_width=True, hide_index=True,
                         column_config={"Link": st.column_config.LinkColumn("Open on Polymarket", display_text="Trade →")})
            ...
            st.warning("No relevant Polymarket markets right now or temporary API issue.")
